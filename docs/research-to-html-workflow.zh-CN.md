# 从调研到 HTML 的制作规范

[English](research-to-html-workflow.md) · **简体中文**

版本：1.4.0<br>
适用范围：行业、产品、市场、平台、商业模式与技术生态等证据型调查。

## 1. 目标

这套流程产出两个互相约束的成果：

1. README.md 是研究事实源，保存问题、证据、推理、限制和引用；
2. index.html 与 index.zh-CN.html 是英文、简体中文阅读界面，负责分层、导航、交互和传播。

HTML 可以重排信息，但不得改变结论强度、删去关键限制或制造未经研究支持的新数字。

## 2. 阶段一：定义问题

研究开始前先写清：

- 要回答的问题、比较、主张或需要支持的决定；
- 读者角色，但不假定读者会亲自执行某个项目；
- 地区、语言、行业和时间边界；
- “成功”“主流”“赚钱”“增长”等词的可操作定义；
- 调查截点与预计失效速度；
- 明确不在范围内的主题。

把大问题改写成 3–7 个可回答的问题。每个问题必须落到证据、有边界的推论或尚未解决的未知项；行动不是调研的默认产物。

### 必备口径

当研究涉及商业结果时，至少区分：

- 公开规模：筹资额、GMV、下载量、用户数；
- 过程成功：达到目标、交付、上线、增长；
- 商业结果：毛利、净利、留存、复购、现金流；
- 无法从公开资料确认的部分。

## 3. 阶段二：建立证据台账

优先级从高到低：

1. 法规、政府、交易所、公司财报、官方统计；
2. 平台帮助中心、规则、费用、项目页和产品文档；
3. 创作者、开发者或公司的一手复盘；
4. 高质量研究机构和行业数据库；
5. 媒体与二手汇总；
6. 搜索摘要、社区讨论和无法核验的转载。

每条关键证据记录以下字段：

| 字段 | 要求 |
|---|---|
| Claim | 它具体支持什么判断 |
| Source | 直接页面，不链接搜索结果 |
| Source type | 官方统计、平台自报、项目页、第三方研究等 |
| Observed at | 访问或统计截点 |
| Geography | 数据适用地区 |
| Caveat | 样本偏差、营销口径、无法验证的成本等 |
| Confidence | 高、中、低及原因 |

### 时效规则

- 费率、政策、榜单、价格、产品状态和平台入口必须在当前调查中重新核验；
- 实时榜单一律写成“截点快照”；
- 历史案例要同时写明当前交付或运营状态；
- 找不到当前一手来源时，明确写“未能核验”，不要拿旧数据顶替。

## 4. 阶段三：分析与反证

先写观察，再写推断：

~~~text
观察：官方页面显示 441 个项目成功。
推断：该渠道仍然有效，但不能由此推断任意新项目都能获得流量。
限制：平台未公开所有项目的净利润与完整获客成本。
~~~

每个核心结论至少检查：

- 是否把相关性写成因果；
- 是否把流水写成利润；
- 是否把平台整体数据套到子品类；
- 是否忽略失败、延期或未交付案例；
- 是否因为少数头部项目产生幸存者偏差；
- 是否把平台营销材料当成独立审计。

遇到冲突数据时，不求简单平均。先检查定义、时间、地区、样本和统计口径，再说明为何采用某一口径。

## 5. 阶段四：编写 Markdown 事实源

每份 README.md 建议使用以下结构：

1. 简洁标题与一句话副标题；
2. 简短调研说明：背景、要回答的具体问题、主要数据来源；
3. 调查截点、范围、口径、方法与限制；
4. 一页结论；
5. 核心概念或计算模型；
6. 平台、市场或竞品对照；
7. 类型、机制或有边界的假设；只有输入可比且规则明确时才排序；
8. 风险、反例与证据缺口；
9. 必要的影响解释或尚未解决的问题；
10. 核心来源。

### 写作规则

- 结论先于过程；
- 每段只承担一个判断；
- 数字附近放直接来源；
- 明确标注平台自报和研究者推断；
- 表格用于精确映射，不把所有段落都塞进表格；
- 不大段复制原文；
- 不用“显然”“必然”“稳赢”等超出证据的词。
- 把受众当作读者，不默认其会亲自开发、购买、启动或运营项目；
- 报告内不放验证计划、项目简报、实施路线、行动评分卡、推荐开发顺序或通用下一步；
- 用户若明确需要建议，另建独立配套产物，并标明其中的证据、推论和价值判断。

Markdown 完成并审校前，不进入 HTML 设计阶段。

## 6. 阶段五：从 Markdown 编排为 HTML

从 `templates/research-report/` 复制页面骨架，并使用稳定的公开英文 slug：

~~~text
reports/<research-slug>/
├─ README.md
└─ README.zh-CN.md

site/<research-slug>/
├─ index.html
├─ index.zh-CN.html
└─ report.js        # 仅在需要报告专属交互时添加
~~~

公开页面必须引用 `site/assets/` 中的公共资产：

~~~html
<link rel="stylesheet" href="../assets/research.css">
<script src="../assets/i18n.js"></script>
<script src="../assets/research.js"></script>
~~~

英文 index.html 是 `x-default` 和不支持语言的兜底；两份页面顶部都提供可抓取的 EN / 中切换。每个语言页面使用稳定的自规范 URL，不按已保存偏好、浏览器或系统语言自动跳转。显式选择可以持久化到控件，但不能改变 URL 对抓取程序的稳定性。两种语言必须保持结论、数字、限制、来源、计算输入、因果推理与不确定性一致。

### 信息层级

1. Hero 先用简洁主题名作为标题，再用副标题写出可验证的中心判断；
2. 结论前用简短区块说明背景、要回答的问题和主要数据来源；
3. 速读层保留结论、关键数字、主要限制与重要未知项；
4. 完整层补充平台表、案例、方法和来源；
5. 每一节先给发现，再给证据；
6. 长表格、来源和补充案例允许按需展开；
7. 页面末尾保留调查截点和研究限制。

### 允许的交互

只有在显著降低理解成本时才添加：

- 阅读深度切换；
- 大字和夜读；
- 章节目录与阅读进度；
- 数据筛选；
- 成本或情景计算器；
- 真实历史顺序所需的时间线；
- 打印版。

交互只能变换已披露的数据与筛选条件，不能把研究者偏好变成推荐。动效不能承载必要信息；减少动画偏好必须得到尊重。

### 禁止事项

- 把完整 CSS 复制进每个 HTML；
- 使用无法离线工作的外部 UI 框架或字体作为必要依赖；
- 为视觉效果增加没有来源的新数字；
- 用颜色作为唯一状态信号；
- 用大量相同卡片代替真实的信息层级；
- 默认隐藏研究限制；
- 在报告中加入验证计划、项目简报、实施路线、行动评分卡、推荐开发顺序或通用下一步；
- 跳过 h1、h2、h3 标题级别。

## 7. 阶段六：视觉与工程验收

### 内容

- HTML 与 README 的结论、数字、截点和限制一致；
- 所有外部链接指向直接来源；
- 速读模式没有隐藏会反转结论的条件；
- 未交付项目没有被写成完整商业成功。

### 结构与无障碍

- 页面只有一个 h1 和一个 main；
- 表单控件都有 label；
- 图像都有有意义的 alt，装饰图像使用空 alt；
- 所有按钮和导航均可用键盘操作；
- 焦点清晰可见；
- 标准正文对比度不低于 4.5:1；
- 不依赖颜色单独表达风险或状态；
- 关闭 JavaScript 后仍能阅读完整正文。

### 响应式

至少验证：

- 320px：无页面级横向滚动，触控目标可用；
- 768px：目录转为抽屉，正文行宽稳定；
- 1024px：布局没有尴尬空洞；
- 1440px：正文不被拉宽，侧栏不抢阅读注意。

宽表格可以局部横向滚动，但必须有可见提示和可聚焦容器。

### 功能

- 控制台无异常；
- 阅读模式、主题和字号可切换；
- 英文兜底、手动语言链接与显式偏好持久化正确，且不存在自动语言跳转；
- 中英文页面的结论、数字、限制、来源与交互输出一致；
- 深链接能正确显示被隐藏的完整层；
- 报告实际使用的计算器与筛选输出正确；
- 打印时显示完整正文与展开内容；
- 共享资源路径在本地文件和 GitHub Pages 上均有效。

## 8. 阶段七：发布

发布前：

1. 更新根 README.md 的调查目录；
2. 更新 `site/index.html` 与 `site/index.zh-CN.html` 的页面入口；
3. 运行 scripts/Validate-ResearchSite.ps1；
4. 从明确白名单构建 Pages 公开产物，确认底稿、docs、skills、scripts、模板、隐藏文件和本地配置均未进入；
5. 验证唯一标题与描述、绝对 canonical 与互指 hreflang、社交元数据、JSON-LD、站点地图、可用的 noindex 404 页，以及可见的方法和纠错入口；
6. 查看 git diff，只暂存本次研究相关文件；
7. 使用简洁提交信息并推送；只有经过明确授权、已验证备份的历史脱敏才使用 force-with-lease。

## 9. Definition of Done

一份调查只有同时满足以下条件才算完成：

- Markdown 事实源可独立阅读；
- 英文与简体中文 HTML 均可独立阅读并可在顶部切换；
- HTML 在无外部依赖时可打开；
- 调研说明、结论、证据、限制和重要未知项完整；
- 共享 CSS 和脚本没有被复制；
- 320–1440px 视觉检查通过；
- 键盘、打印、无脚本退化通过；
- 仓库目录与验证脚本通过；
- 根目录调查索引已更新。
