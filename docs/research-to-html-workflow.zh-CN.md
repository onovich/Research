...
2. index.html 与 index.zh-CN.html 是英文、简体中文阅读与决策界面，负责分层、导航、交互和传播。
...
英文 index.html 是 `x-default` 和不支持语言的兜底；两份页面顶部都提供可抓取的 EN / 中切换。每个语言页面使用稳定的自规范 URL，不按已保存偏好、浏览器或系统语言自动跳转。显式选择可以持久化到控件，但不能改变 URL 对抓取程序的稳定性。两种语言必须保持结论、数字、限制、来源、计算阈值与决策逻辑一致。
