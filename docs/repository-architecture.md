...
|---|---|---|
| `/crowdfunding-and-indie-games-research/` | Pillar report | Crowdfunding economics, product/platform patterns, and indie-game channel evidence |
| `/indie-game-crowdfunding-genres-and-gameplay/` | Companion deep dive | Current game-platform snapshots, genre/gameplay patterns, released cases, failure modes, platform differences, and observed fit signals |
| `/tools/research-to-html/` | Main Skill product page | Turn an evidence-backed research question into a bilingual HTML report |

The two reports are not test duplicates. They reuse one evidence foundation, but detailed ownership is exclusive: the pillar owns economics and high-level channel evidence; the companion owns the game taxonomy, cases, risks, platform differences, evidence filter, and observed-signal table. Keep both pages and reciprocal links, but do not copy those detailed sections back into the pillar or create near-identical pages for keyword variants.
